...
function displayHourlyForecast(hourlyData) {
    ...
    const next24Hours = hourlyData.slice(0, 8); // display next 24 hours

    next24Hours.forEach(item => {

        const dateTime = new Date(item.dt * 1000); //conversion to milliseconds
        const hour = dateTime.getHours();
        const temperature = Math.round(item.main.temp - 273.15); //celsius conversion
